		...
		while($row = $resultMe->fetch_assoc())
		 {
			...
			$SID = $row['idSurvey'];
			$ResearchSurvey.= "<div class =\"innerRow selectRow\"><span class=\"ninety\" onclick =\"testSelect($SID)\">$Survey</span><span class=\"X\" onclick =\"Sure2('$SID')\">X</span></div>";
		}
	}			
	//QUaltrics survey
		...
		while($row = $resultMe->fetch_assoc())
		 {
			...
			$SID = $row['idSurvey'];
			$Link = $row['Link'];
			$QualticsSurvey.= "<div class =\"innerRow selectRow\"><span class=\"ninety\"  onclick =\"redirect('$Link','$SID')\">$Survey</span><span class=\"X\" onclick =\"Sure2('$SID')\">X</span></div>";
		}
	}
...
function Sure2(SID){
if (confirm('Are you sure you want to remove the study from your list?')) {
	refresh(SID)
} else {
    // Do nothing!
}
